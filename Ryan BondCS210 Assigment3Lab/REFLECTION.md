...
node pointer itself. First, a new int is created as the argument for addFront in main.cpp, then a new Node is created 
inside the addFront function in LinkedList.h; finally, in deleteFront, doomed is set to point to the head node as a 
temporary variable, and head is moved to head->next. Then doomed is destroyed.
...
Since the functions serve different purposes, the pointers must be treated differently.
In addFront, the T* allocates a new node in memory and is held in a temporary variable before being assigned to the list.
In search, T* is used merely as a key to find a matching item in the list.
...
The two mechanisms that made this possible are polymorphism and template classes.
...
evaluate the function at runtime, choosing which function to call based on the context in the code. If the compiler 
cannot find an implementation, the program will compile, but it will call the wrong function.